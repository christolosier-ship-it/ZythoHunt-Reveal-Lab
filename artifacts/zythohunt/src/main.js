...
function addCarouselPanel(carousel) { const title = document.createElement("h3"); title.className = "debug-section-title"; title.textContent = "Carrousel"; debugBody.appendChild(title); CAROUSEL_CONTROLS.forEach(([key, label, min, max, step, unit]) => { const row = document.createElement("div"); row.className = "debug-row"; row.innerHTML = `<label class="debug-label" for="csl-${key}">${label}</label><div class="debug-input-row"><input class="debug-range" id="csl-${key}" type="range" min="${min}" max="${max}" step="${step}" value="${carouselTokens[key]}"><span class="debug-value">${carouselTokens[key]}${unit}</span></div>`; const input = row.querySelector("input"), value = row.querySelector("span"); input.addEventListener("input", () => { const v = parseFloat(input.value); updateCarouselToken(key, v); value.textContent = `${v}${unit}`; carousel.refresh(); }); debugBody.appendChild(row); }); }
function rebuildDebugPanel(carousel) { debugBody.innerHTML = ""; const title = document.createElement("h3"); title.className = "debug-section-title"; title.textContent = "Révélation"; debugBody.appendChild(title); createDebugPanel(debugBody, () => {}); addCarouselPanel(carousel); }
...
async function boot() {
  ...
  carousel.mount(); rebuildDebugPanel(carousel);
  ...
  $("debug-reset-settings").addEventListener("click", () => { resetDebugPanel(debugBody, () => {}); rebuildDebugPanel(carousel); });
  $("debug-reset-carousel").addEventListener("click", () => { resetCarouselTokens(); rebuildDebugPanel(carousel); carousel.refresh(); });
  $("debug-reset-all").addEventListener("click", async () => { await carousel.closeInspection(); resetCarouselTokens(); resetDebugPanel(debugBody, () => {}); rebuildDebugPanel(carousel); carousel.refresh(); });
  $("debug-replay").addEventListener("click", async () => { if (revealEngine.isBusy()) return; const id = carousel.getActiveCardId(); const card = revealablePrototypeCards.find((c) => c.id === id); if (!card) return; closeSettings(); carousel.lock(); try { const result = await revealEngine.reveal({ cardEl: carousel.getCardElement(id), cardData: card, sceneContext: carousel.createRevealContext(id), mode: "full" }); if (result.status === "completed") await revealEngine.returnToSource(); } finally { carousel.unlock(); } });
